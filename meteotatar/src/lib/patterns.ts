...
/**
 * Ensures the map's SVG renderer has a <defs> node we control, then lazily
 * creates (and caches) a <pattern> element for the requested fill type +
 * color combination. Returns the pattern id so it can be referenced via
 * url(#id).
 */
...
export function applyShapeStyle(
  ...
  if (style.fillType === "solid") {
    ...
    if (patternId) path.setAttribute("fill", `url(#${patternId})`);
  }
}
