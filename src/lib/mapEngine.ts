...
const HINTS: Partial<Record<DrawMode, string>> = {
  ...
  rectangle: "Проведите мышью/пальцем по карте, чтобы задать прямоугольник.",
...
export class MapEngine {
  ...
  setMode(mode: DrawMode) {
    ...
    if (mode === "edit" && this.selectedId) this.buildEditHandles(this.selectedId);
  ...
  private addDraftPoint(latlng: L.LatLng) {
    ...
    if (this.draftPoints.length === 1) {
      marker.on("click", (evt) => {
        ...
        if (this.mode === "polygon" && this.draftPoints.length >= 3) this.finishDrawing();
      });
    }
  ...
  private redrawDraft() {
    ...
    if (this.mode === "polygon") {
      this.draftLayer = L.polygon(this.draftPoints, {
      ...
      this.draftLayer = L.polyline(this.draftPoints, {
        color: this.activeStyle.color,
        weight: 2,
        dashArray: "6,6",
      }).addTo(this.map);
  ...
  finishDrawing() {
    ...
    if (this.draftPoints.length >= min) {
      if (this.mode === "polygon") this.commitShape("polygon", this.draftPoints.slice());
      else if (this.mode === "line") this.commitShape("line", this.draftPoints.slice());
    }
    this.resetDraft();
  }
  ...
  // ---------- Pointer-drag tools: rectangle, circle, freehand ----------
  private handlePointerDown = (ev: PointerEvent) => {
    ...
    if (this.mode === "freehand") this.draftPoints = [latlng];
  };
  ...
  private commitShape(
    ...
    style: ShapeStyle = this.activeStyle
  ) {
    const id = uid();
    let layer: L.Path;
    if (kind === "line") layer = L.polyline(points, {});
    else layer = L.polygon(points, {});
  ...
  exportGeoJSON(): string {
    const features = this.order.map((id) => {
      ...
      if (rec.kind === "circle" || rec.kind === "point") {
        ...
        if (rec.kind === "line") geometry = { type: "LineString", coordinates: coords };
        else geometry = { type: "Polygon", coordinates: [[...coords, coords[0]]] };
      }
      return {
        type: "Feature",
        properties: { id: rec.id, name: rec.name, categoryId: rec.categoryId, style: rec.style, radius: rec.radius },
        geometry,
      };
    });
  ...
  importGeoJSON(json: string) {
    try {
      ...
      const snaps: Snapshot[] = (data.features ?? []).map(
        (f: {
  ...
  emitChange() {
    const list: ShapeRecord[] = this.order.map((id) => {
      const rec = this.shapes.get(id)!;
      return { id: rec.id, kind: rec.kind, name: rec.name, categoryId: rec.categoryId, style: rec.style, visible: rec.visible };
    });
